...
const test = {
	start(){
		...
		client = new Worker(
			'http://www.bsidesoft.com/hika/s66/15_1/', 
			_=>{
	...
	priceAfterBidding(){
		client.js('return runner.showsSniperIsWinning()').
		then(v=>{
			reporter.assert('runner.showsSniperIsWinning()', v, true);
	...
	wonAuction(){
		client.js('return runner.showsSniperHasWonAuction()').
		then(v=>reporter.assert('runner.showsSniperHasWonAuction()', v, true));
	}
};
test.start();