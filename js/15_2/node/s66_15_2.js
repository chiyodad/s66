...
const test = {
	start(){
		...
		client = new Worker(
			'http://www.bsidesoft.com/hika/s66/15_2/', 
			_=>{
	...
	price(){
		client.js('return runner.showsSniperIsBidding(1000, 1098)').
		then(v=>{
			reporter.assert('runner.showsSniperIsBidding(1000, 1098)', v, true);
	...
	bidding(){
		...
		client.js('Connection.pushed()').
		then(_=setTimeout(test.priceAfterBidding, 1000));
	},
	priceAfterBidding(){
		client.js('return runner.showsSniperIsWinning(1098)').
		then(v=>{
			reporter.assert('runner.showsSniperIsWinning(1098)', v, true);
			auction.announceClosed();
			client.js('Connection.pushed()').
			then(_=setTimeout(test.wonAuction, 1000));
		});
	},
	wonAuction(){
		client.js('return runner.showsSniperHasWonAuction(1098)').
		then(v=>reporter.assert('runner.showsSniperHasWonAuction(1098)', v, true));
	}
};
test.start();